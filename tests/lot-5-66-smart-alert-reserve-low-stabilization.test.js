...
const BOUNDARY_SCENARIOS = Object.freeze([
  { id: "charges-zero", currentMonthTotal: 0, chargesInput: 0, expected: "ALERT_OFF" },
  { id: "low-amount", currentMonthTotal: 25, chargesInput: 6, expected: "ALERT_OFF" },
...
const PRODUCTION_SCENARIOS = Object.freeze([
...
test("LOT 5.66 keeps the reserve-low Shadow source stable", () => {
...
test("LOT 5.66 keeps Shadow baseline at thirteen after approved revenue input migration", () => {
  const app = sourceWithoutComments(APP_SOURCE);
  const smartCall = sourceWithoutComments(smartAlertsBlock());
...
test("LOT 5.66 keeps reserve-low threshold, ON/OFF logic and availableAmount stable", () => {
  ...
  assert.doesNotMatch(smartFunction, /availableAmount|smartAlertEstimatedCharges|fiscalSummaryVisibleSlice/);
  assert.match(app, /return Math\.max\(0, currentMonthTotal - estimatedCharges\);/);
});

test("LOT 5.66 keeps reserve-low output and priority stable", () => {
  const smartFunction = sourceWithoutComments(buildSmartAlertsFunctionBlock());
...
test("LOT 5.66 stabilizes zero, low and threshold boundaries", () => {
  assert.deepEqual(
...
test("LOT 5.66 stabilizes threshold transitions", () => {
  const below = reserveLowState({ currentMonthTotal: 199, chargesInput: 100 });
  const exact = reserveLowState({ currentMonthTotal: 200, chargesInput: 100 });
  const above = reserveLowState({ currentMonthTotal: 201, chargesInput: 100 });

  assert.deepEqual([below, exact], ["ALERT_ON", "ALERT_OFF"]);
  assert.deepEqual([exact, above], ["ALERT_OFF", "ALERT_OFF"]);
  assert.deepEqual([above, below], ["ALERT_OFF", "ALERT_ON"]);
});

test("LOT 5.66 stabilizes revenue and ACRE production scenarios", () => {
  assert.deepEqual(
    PRODUCTION_SCENARIOS.map((scenario) => [scenario.id, reserveLowScenarioResult(scenario)]),
    PRODUCTION_SCENARIOS.map((scenario) => [scenario.id, scenario.expected]),
  );
});

test("LOT 5.66 keeps deterministic same input and cloned input behavior", () => {
...
test("LOT 5.66 keeps feature flag ON and OFF stable without local fallback", () => {
...
test("LOT 5.66 keeps estimatedCharges Legacy elsewhere; confirms root savingsGoal removed", () => {
  const app = sourceWithoutComments(APP_SOURCE);
  ...
  assert.match(exportSource, /estimatedCharges|dashboardChargesDisplay|savingsGoal/);
  assert.match(assistant, /realMonthlyRevenue: currentMonthTotal/);
  assert.match(feedback, /totalRevenues: currentMonthTotal \|\| 0/);
});

test("LOT 5.66 keeps other smart alerts unchanged and isolated", () => {
  const smartFunction = sourceWithoutComments(buildSmartAlertsFunctionBlock());
...
test("LOT 5.66 keeps no propagation to adjacent boundaries", () => {
  const smartCall = sourceWithoutComments(smartAlertsBlock());
  const coaching = sourceWithoutComments(coachingBlock());
  const exportSource = sourceWithoutComments(exportBlock());
  const assistant = sourceWithoutComments(assistantGuidanceBlock());
  ...
  assert.doesNotMatch(
    smartCall,
    /supabase|localStorage|sessionStorage|payload|feedback|analytics|export|assistant|coaching|savingsGoal/i,
  );
...
test("LOT 5.66 keeps React and pipeline surfaces stable", () => {
  const app = sourceWithoutComments(APP_SOURCE);
  const smartCall = sourceWithoutComments(smartAlertsBlock());
...
test("LOT 5.66 keeps parity and runtime evidence intact", () => {
  assert.match(LOT_5_61_SOURCE, /strict input MATCH/);
  assert.match(LOT_5_65_SOURCE, /validates boundary scenarios with unchanged ON\/OFF behavior/);
...
test("LOT 5.66 keeps rollback local to the reserve-low input", () => {
